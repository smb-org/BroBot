...
const requestFor = async (userId: string, channelId: string, moduleId: string): Promise<Request> => {
  ...
  return new Request(`https://brobot.example/auth/channels/${channelId}/broadcaster-scopes/${moduleId}`, {
...
describe("Broadcaster-Scope-Route", () => {
  ...
  it("behält Scopes bereits aktivierter eigener Broadcaster-Module bei", async () => {
    ...
    const response = await authRouter.fetch(await requestFor("user-1", "kanal-a", "textbefehle"), environment(database));
    const location = new URL(response.headers.get("location") ?? "https://invalid");
    const scopes = location.searchParams.get("scope")?.split(" ") ?? [];
  ...
  it("liefert für ein unbekanntes Registry-Modul 404 ohne Autorisierung", async () => {
    ...
    const response = await authRouter.fetch(await requestFor("user-1", "kanal-a", "unbekannt"), environment(database));
