...
describe("Schema-Baseline", () => {
  ...
  const dateien = readdirSync(verzeichnis).filter((name) => name.endsWith(".sql")).sort();

  it("hält LATEST_SCHEMA_MIGRATION an der letzten Migrationsdatei", () => {
    expect(dateien.at(-1)).toBe(LATEST_SCHEMA_MIGRATION);
  });

  it("legt die von /healthz geprüfte Tabelle wirklich an", () => {
    const baseline = readFileSync(resolve(verzeichnis, LATEST_SCHEMA_MIGRATION), "utf8");
