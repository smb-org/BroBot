...
describe("Dashboard-Grundgerüst", () => {
  ...
  it("zeigt Abo-Stufen im Zahl-Chip als T1/T2/T3/Prime statt roh, unbekannte Werte ohne Chip", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      ...
      if (path.endsWith("/events")) return jsonResponse({
        entries: [
          { eventId: "t1", createdAt: "2026-09-18T04:00:00.000Z", moduleId: "channel_events", code: "channel_events.chat.sub", detail: '{"person":"tier1","stufe":"1000"}', actorUserId: null },
          { eventId: "t2", createdAt: "2026-09-18T04:01:00.000Z", moduleId: "channel_events", code: "channel_events.chat.sub", detail: '{"person":"tier2","stufe":"2000"}', actorUserId: null },
          { eventId: "t3", createdAt: "2026-09-18T04:02:00.000Z", moduleId: "channel_events", code: "channel_events.chat.sub", detail: '{"person":"tier3","stufe":"3000"}', actorUserId: null },
          { eventId: "prime", createdAt: "2026-09-18T04:03:00.000Z", moduleId: "channel_events", code: "channel_events.chat.sub", detail: '{"person":"primeperson","stufe":"Prime"}', actorUserId: null },
          { eventId: "unbekannt", createdAt: "2026-09-18T04:04:00.000Z", moduleId: "channel_events", code: "channel_events.chat.sub", detail: '{"person":"rätselperson","stufe":"9999"}', actorUserId: null },
        ],
        nextCursor: null,
      });
      return jsonResponse({}, 404);
  ...
  it("zeigt aktive Filter, kombiniert sie und meldet einen Treffer-Leerzustand", async () => {
    ...
    fireEvent.change(modul, { target: { value: "text_commands" } });
    ...
    fireEvent.change(modul, { target: { value: "ads" } });
  ...
  it("behält den erfolgreichen Reload-Stand gegen eine verspätete Erstantwort", async () => {
    ...
    const verspäteterMember = { userId: "late-user", login: "late-user", displayName: "Verspäteter Stand", profileImageUrl: null, role: "operator", joinedAt: "2026-09-18T00:00:00.000Z" };
  ...
  it("verweist in der Kanalübersicht auf aktive Module statt ihre Formulare einzubetten", async () => {
    const channel = healthyChannel("kanal-a", "Alpha");
    const aktivesModul = { ...overview(channel), activeModules: [{ moduleId: "text_commands", settings: "{}" }] };
    vi.stubGlobal("fetch", vi.fn((input: RequestInfo | URL) => {
      const path = requestUrl(input).pathname;
      if (path === "/api/channels") return jsonResponse({ channels: [channel] });
  ...
  it("lädt die Kanalübersicht nach dem Einschalten neu und zeigt die Modulansicht", async () => {
    ...
    const fetcher = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === "/api/channels/kanal-a/modules" && init?.method === undefined) {
        modulesAufrufe += 1;
        return jsonResponse({ modules: [{ id: "text_commands", enabled: modulesAufrufe > 1, settings: "{}" }] });
      }
      if (url.pathname === "/api/csrf") return jsonResponse({ token: "csrf-token" });
