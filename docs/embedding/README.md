...
Die Widget-Dateien liegen unter [`streamelements/`](streamelements/). In StreamElements ein Overlay öffnen und *Add Widget → Static/Custom → Custom Widget* wählen. `widget.html` in den HTML-Reiter, `widget.js` in JS und `fields.json` in Fields kopieren; den CSS-Reiter leer lassen. Im Feld *Overlay-URL* den vollständigen BroBot-Link mit Zugangstoken eintragen. Das Widget füllt seine Box mit dem Overlay. Bei einem einzelnen Element die Box im Editor positionieren; für das ganze Overlay die Box auf die Overlay-Fläche setzen.
