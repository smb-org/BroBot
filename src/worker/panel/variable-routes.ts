...
import {
  channelVariableOverlayIdsByName,
  findChannelVariable,
  listChannelVariables,
  prepareChannelVariableOverlayReferences,
  type ChannelVariableOverlayReference,
...
variableRouter.patch("/api/channels/:channelId/variables/:name", async (context) => {
  ...
  const results = await context.env.DB.batch([references, mutation, audit, ...overlayRevisionUpdates, ...referenceUpdates]);
  ...
  const overlayRows = newName === name ? [] : (results[3]?.results ?? []) as Array<{ overlay_id: string; revision: number }>;
  const overlayMessages = overlayRows.map((row) => overlayChangedMessage(channelId, row.overlay_id, row.revision));
  const referencesByOldName = channelVariableOverlayIdsByName(
    [name, newName], (results[0]?.results ?? []) as ChannelVariableOverlayReference[],
  );
